/**
 * LandingPage.jsx — with toast notifications + loading states
 */
...
function Feat({ icon, text }) {
  return (
    <motion.div whileHover={{ x: 4 }} style={{
      ...
      padding: '7px 14px', background: C.faint, border: `1px solid ${C.border}`,
...
export default function LandingPage() {
  ...
  return (
    <div
      ref={containerRef} onMouseMove={handleMouse}
      style={{ height: '100vh', overflow: 'hidden', background: C.bg, display: 'flex', fontFamily: "'Outfit', sans-serif", position: 'relative' }}
    >
      {/* Grid */}
      ...
      <motion.div style={{ position: 'absolute', width: 360, height: 360, borderRadius: '50%',
        ...
        pointerEvents: 'none', zIndex: 1, x: glowX, y: glowY }} />
      <div style={{ position: 'absolute', top: '-8%', left: '-4%', width: 520, height: 520, background: `radial-gradient(circle, rgba(0,229,255,0.05) 0%, transparent 65%)`, borderRadius: '50%', pointerEvents: 'none' }} />

      {/* LEFT */}
      <motion.div variants={stagger} initial="hidden" animate="visible"
        style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 52px', position: 'relative', zIndex: 10 }}>
        <motion.div variants={up} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 36 }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, color: C.text, letterSpacing: '0.12em' }}>COLLABCODE</span>
        </motion.div>

        <motion.div variants={up} style={{ marginBottom: 20 }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: C.accent, letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ display: 'inline-block', width: 20, height: 1, background: C.accent }} /> Real-time collaboration
          </div>
          <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(48px, 6.5vw, 96px)', lineHeight: 0.9, color: C.text, margin: '0 0 20px', letterSpacing: '0.01em' }}>
            CODE<br /><span style={{ color: C.accent }}>TOGETHER.</span><br />SHIP FASTER.
          </h1>
        </motion.div>

        <motion.p variants={up} style={{ fontSize: 14, color: C.muted, lineHeight: 1.65, maxWidth: 380, margin: '0 0 28px' }}>
      ...
      {/* RIGHT — Auth card */}
      <div style={{ width: 440, flexShrink: 0, height: '100%', overflowY: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: '32px 40px', borderLeft: `1px solid ${C.border}`,
        background: 'rgba(11,14,20,0.88)', backdropFilter: 'blur(32px)', position: 'relative', zIndex: 10, boxSizing: 'border-box' }}>
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1], delay: 0.18 }} style={{ width: '100%' }}>
          {/* Tab toggle */}
          <div style={{ display: 'flex', background: C.faint, borderRadius: 12, padding: 3, border: `1px solid ${C.border}`, marginBottom: 28 }}>
            {['Sign In', 'Sign Up'].map((t, i) => {
              ...
              return (
                <button key={t} onClick={() => { setIsLogin(i === 0); }} style={{
                  ...
                  background: active ? C.accent : 'transparent', color: active ? '#080a0f' : C.muted, transition: 'all 0.18s',
                }}>{t}</button>
              );
            })}
          </div>
          ...
          <div style={{ marginBottom: 24 }}>
            <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 38, color: C.text, margin: '0 0 4px', letterSpacing: '0.02em' }}>
              {isLogin ? 'WELCOME BACK' : 'JOIN THE TEAM'}
            </h2>
            <p style={{ fontSize: 12, color: C.muted }}>{isLogin ? 'Sign in to your workspace.' : 'Create your CollabCode account.'}</p>
          </div>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <AnimatePresence>
              {!isLogin && (
                <motion.div key="uname" initial={{ opacity: 0, height: 0, overflow: 'hidden' }} animate={{ opacity: 1, height: 'auto', overflow: 'visible' }} exit={{ opacity: 0, height: 0, overflow: 'hidden' }} transition={{ duration: 0.22 }}>
            ...
            <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.012 }} whileTap={{ scale: 0.985 }}
              style={{
                ...
                boxShadow: `0 0 22px rgba(0,229,255,0.2)`, transition: 'background 0.18s',
              }}>
              {loading ? (
                <span style={{ width: 15, height: 15, border: '2px solid rgba(8,10,15,0.3)', borderTopColor: '#080a0f', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} />
              ) : (
                <>{isLogin ? 'Enter Workspace' : 'Create Account'} <ArrowRight size={15} strokeWidth={2.5} /></>
              )}
            </motion.button>
          </form>
          ...
          <p style={{ textAlign: 'center', fontSize: 12, color: C.muted, margin: 0 }}>
            ...
            <button onClick={() => setIsLogin(!isLogin)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.accent, fontWeight: 600, fontSize: 12, fontFamily: "'Outfit', sans-serif", padding: 0 }}>
